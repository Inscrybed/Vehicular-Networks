...
my_system_in_queue = Queue()     # RXD → RSU_SYSTEM
my_system_out_queue = Queue()    # RSU_SYSTEM → TXD
...
def main(argv):
	...
	try:
		...
		if (node_type==maps.obu_node):
			...
			print("OBU create and send\n")
			t=Thread(target=obu_application_txd, args=(node_interface, start_flag, my_system_rxd_queue, ca_service_txd_queue, den_service_txd_queue,coordinates,))
			t.start()
			threads.append(t)
		...
		if (node_type==maps.rsu_node):
			...
			# 			  	ca_service_txd_queue: queue to send data to ca_services_txd
			#             	den_service_txd_queue: queue to send data to den_services_txd
			t=Thread(target=rsu_application_txd, args=(node_interface, start_flag, my_system_out_queue, ca_service_txd_queue, den_service_txd_queue, ivim_service_txd_queue, spat_service_txd_queue,))
			t.start()
			threads.append(t)
			...
			#             	services_rxd_queue: queue to get data from ca_service_rxd or den_service_rxd
			#             	my_system_rxd_queue: queue to send data to my_system that is relevant for business logic decision-process 
			t=Thread(target=rsu_application_rxd, args=(node_interface, start_flag, services_rxd_queue, my_system_in_queue,))
			t.start()
			threads.append(t)
			...
			#	   		  	my_system_rxd_queue: queue to receive data from other application layer threads relevant for business logic decision-process 
			#             	movement_control_txd_queue: queue to send commands to control vehicles movement
			t=Thread(target=rsu_system, args=(node_interface, start_flag, coordinates, my_system_in_queue, rsu_control_txd_queue, my_system_out_queue))
			t.start()
			threads.append(t)
