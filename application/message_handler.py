...
from application.event_config import EventConfig
...
#						(roi_x, roi_y) 
#-------------------------------------------------------------------------------------------------
'''def trigger_event(node_type, event_number, status_number, intensity_level, x, y):

	#if (node_type == map.obu_node):
	
	event_type = app_obu_conf.event_type[event_number]
	event_status = app_obu_conf.event_type[status_number]
	rep_interval = app_obu_conf.rep_interval[0] 
	n_hops = app_obu_conf.n_hops[0]
	roi_x = app_obu_conf.roi_x[0] 
	roi_y = app_obu_conf.roi_y[0]
	latency = app_obu_conf.latency[0]
	intensity = app_obu_conf.intensity[intensity_level]
	...
	# 	intensity = app_obu_conf.intensity[event_number]

	event_msg={'event_type': event_type, 
			'event_status': event_status, 
			'rep_interval':int(rep_interval), 
			'n_hops': int(n_hops), 
			'roi_x':int(roi_x), 
			'roi_y': int(roi_y), 
			'latency':int(latency), 
...
def trigger_event(node_id, event: EventConfig) -> Dict:
		"""Format event as DEN message"""
		print("Trigger Event\n")
		return {
			'msg_type': 'DEN',
			'event_id': event.event_id,
			'event_type': event.event_type.value,
			'hazard_subtype': event.hazard_subtype.value if event.hazard_subtype else None,
			...
			'repetition_interval': event.repetition_interval,
			'max_hops': event.max_hops,
			'max_latency': event.max_latency
		}
def trigger_situation(node_id, event: EventConfig) -> Dict:
		"""Format event as IVIM message"""
		print("Trigger Event\n")
		return {
			'msg_type': 'IVIM',
			'event_id': event.event_id,
